...
class MessageService(_service_base.ServiceBase):
    async def list_active_marketplace_messages(
        ...
        """Returns active PSS marketplace messages for a user from the ``MessageService/ListActiveMarketplaceMessages5`` endpoint."""
    ...
    async def list_messages_for_channel_key(self, access_token: str, channel_key: str) -> _List[_Message]:
        """Returns PSS messages for the specified channel from the ``MessageService/ListMessagesForChannelKey`` endpoint."""
        production_server = await self.get_production_server()
    ...
    async def list_private_messages(self, access_token: str) -> _List[_Message]:
        """Returns the user's PSS private messages from the ``MessageService/ListPrivateMessages`` endpoint."""
        production_server = await self.get_production_server()
    ...
    async def send_private_message(self, access_token: str, message: str, to_user_id: int) -> _List[_Message]:
        """Returns the resulting PSS private messages from the ``MessageService/SendPrivateMessage3`` endpoint."""
        production_server = await self.get_production_server()
    ...
    async def send_message(self, access_token: str, channel_key: str, message: str) -> _Message:
        """Returns the PSS message created by the ``MessageService/SendMessage3`` endpoint."""
        production_server = await self.get_production_server()
